...
def sample_df() -> pd.DataFrame:
    ...
    # 10 条样本，5 no + 5 yes，确保 stratify 的每个 class ≥ 2
    return pd.DataFrame(
        {
            "id": list(range(1, 11)),
...
def test_build_pipeline():
    """_build_pipeline 应返回正确的 Pipeline。"""
...
def test_train_and_predict(csv_with_sample: Path, tmp_path: Path):
    """训练模型后，应能正确预测并返回指标。"""
    model_path = tmp_path / "model.joblib"
    # 用 monkey-patch 的方式修改 MODEL_PATH
...
def test_load_model_not_found(tmp_path: Path):
    """模型不存在时，load_model 应返回 None。"""
...
def test_predict_without_model():
    ...
    result = predict(sample, model=None)
